...
- **Server-Side**: \`web_search\`, \`web_extract\`, \`semantic_code_navigator\` (with line numbers),\`grep_search\`, \`check_dev_errors\`, \`test_app\`, \`list_files\` (client sync), \`read_file\` (client sync) and \`continue_backend_implementation\`
- **App Testing**: \`test_app\` (MANDATORY after building/enhancing features - starts dev server, tests all routes, runs build, reports results)
...
PHASE 5: MANDATORY TESTING (NON-NEGOTIABLE)
→ ALWAYS use the \`test_app\` tool after completing your implementation
→ \`test_app\` starts the dev server, tests all page routes & API routes, checks for runtime errors, and runs the production build
→ If any tests fail: fix the issues and run \`test_app\` again (max 2 retries)
→ Only provide your final summary AFTER all tests pass
→ Include test results in your summary: "Test Results: X/Y passed | Build: OK/FAIL"
→ NEVER tell the user the app is complete without running \`test_app\` first
...
- **ALWAYS use \`test_app\` after building or enhancing features** - it starts the dev server, tests all routes, checks for runtime errors, and verifies the build compiles
- If any tests fail: fix the issues and run \`test_app\` again (max 2 retries)
- Use \`check_dev_errors\` (build mode) for quick build-only checks
...
- **Server-Side**: \`web_search\`, \`web_extract\`, \`semantic_code_navigator\` (with line numbers),\`grep_search\`, \`check_dev_errors\`, \`test_app\`, \`list_files\` (client sync), \`read_file\` (client sync)
- **App Testing**: \`test_app\` (MANDATORY after building/enhancing features - starts dev server, tests all routes, runs build, reports results), \`check_dev_errors\`, \`browse_web\`, \`node_machine\`
...
═══════════════════════════════════════════════════════════════
MANDATORY: TEST AFTER EVERY BUILD (NON-NEGOTIABLE)
═══════════════════════════════════════════════════════════════

**After building, adding, or enhancing ANY feature, you MUST test the app before providing your final summary.** This is a mandatory quality gate that ensures every app you deliver is fully functional.

### Testing Protocol (REQUIRED after every implementation):

**Step 1: Use \`test_app\` tool** to run the comprehensive test suite:
- It automatically starts the dev server, discovers all routes, tests each page and API route, checks for runtime errors, and runs the production build
- Review the test results carefully

**Step 2: If any tests FAIL:**
- Fix the errors in the relevant files using \`edit_file\` or \`write_file\`
- Run \`test_app\` again to verify the fixes work
- Repeat until ALL tests pass (max 2 retry cycles)

**Step 3: Only after ALL tests pass**, provide your summary to the user with the test results included

### What \`test_app\` checks:
- Dev server starts without errors
- All page routes load with HTTP 200 (no blank pages, no error pages)
- All API routes respond correctly (for Next.js projects)
- No runtime/console errors from the dev server
- Production build compiles successfully

### When to test:
- After building a NEW app from scratch
- After adding a new page, feature, or component
- After fixing a bug (to verify the fix and no regressions)
- After any significant code change

### Test results in your summary:
Always include a brief test report at the end of your response:
\`\`\`
Test Results: X/Y passed | Build: OK/FAIL
Routes tested: /, /about, /dashboard, /api/users
\`\`\`

**NEVER skip testing. NEVER tell the user the app is complete without running \`test_app\` first.**
...
- **Mandatory Testing**: ALWAYS use the \`test_app\` tool after building or enhancing features to verify the app loads correctly, all pages work, and the build compiles successfully
- **Build Checks**: Use \`check_dev_errors\` in build mode if you need a quick build-only check
...
>>>>>>> REPLACE`),
      ...
      test_app: tool({
        description: `Comprehensive App Testing Tool - Starts the dev server, navigates to all detected pages and API routes to verify they load correctly with no errors, then runs the production build to check compilation. Use this tool AFTER building, adding, or enhancing any feature to ensure the app is fully functional.

This tool automatically:
1. Detects project type (Next.js, Vite, Expo)
2. Installs dependencies and starts the dev server
3. Discovers all routes from the project file structure
4. Tests each page route with HTTP requests (checks for 200 status, valid HTML, no error pages)
5. Tests each API route (for Next.js projects)
6. Monitors dev server stderr for runtime errors
7. Runs the production build to verify compilation
8. Returns a detailed test report with pass/fail status for each route

IMPORTANT: Use this tool after completing feature implementation to verify the app works end-to-end before providing your summary to the user.`,
        inputSchema: z.object({
          additionalRoutes: z.array(z.string()).optional().describe('Additional routes to test beyond auto-detected ones (e.g., ["/dashboard", "/settings", "/api/users"])'),
          skipBuild: z.boolean().optional().describe('Skip the production build check (default: false)'),
          testFormSubmissions: z.boolean().optional().describe('If true, test form submissions on detected pages using curl POST requests (default: false)')
        }),
        execute: async ({ additionalRoutes = [], skipBuild = false, testFormSubmissions = false }, { abortSignal, toolCallId }) => {
          const toolStartTime = Date.now();
          const timeStatus = getTimeStatus();
          ...
          if (timeStatus.isApproachingTimeout) {
            return {
              success: false,
              error: `App testing cancelled due to timeout warning: ${timeStatus.warningMessage}`,
              toolCallId,
              executionTimeMs: Date.now() - toolStartTime,
              timeWarning: timeStatus.warningMessage
            }
          ...
          try {
            ...
            if (!e2bApiKey) {
              const executionTime = Date.now() - toolStartTime;
              toolExecutionTimes['test_app'] = (toolExecutionTimes['test_app'] || 0) + executionTime;
              return {
                success: false,
                error: 'E2B API key not configured',
                toolCallId,
            ...
            // Get all files from in-memory session store
            const sessionData = sessionProjectStorage.get(projectId)
            if (!sessionData) {
              const executionTime = Date.now() - toolStartTime;
              toolExecutionTimes['test_app'] = (toolExecutionTimes['test_app'] || 0) + executionTime;
            ...
            const logs: string[] = []
            const errors: string[] = []
            const testResults: { route: string; type: string; status: 'pass' | 'fail' | 'warning'; statusCode?: number; message: string; responseTime?: number }[] = []
            ...
            if (packageJsonFile) {
              try {
                ...
                console.warn('[test_app] Failed to parse package.json')
              }
            }
            ...
            const isNextProject = allFiles.some((f: any) =>
              f.path === 'next.config.js' ||
              f.path === 'next.config.ts' ||
              f.path === 'next.config.mjs' ||
              (packageJson && packageJson.dependencies && packageJson.dependencies['next'])
            )

            const projectType = isExpoProject ? 'Expo' : isNextProject ? 'Next.js' : 'Vite'
            const template = "pipilot-expo"
            const workingDir = isExpoProject ? "/home/user" : "/project"

            logs.push(`[TEST] Detected project type: ${projectType}`)

            // ──────────────────────────────────────────────────────────
            // STEP 1: Auto-detect routes from project files
            // ──────────────────────────────────────────────────────────
            const pageRoutes: string[] = ['/']
            const apiRoutes: string[] = []

            if (isNextProject) {
              // Next.js App Router: scan app/*/page.tsx files
              for (const file of allFiles) {
                const path = file.path || ''
                // Match app/page.tsx, app/about/page.tsx, app/dashboard/settings/page.tsx etc.
                const appRouterMatch = path.match(/^app\/(.+\/)?page\.(tsx?|jsx?)$/)
                if (appRouterMatch) {
                  const routePath = appRouterMatch[1]
                    ? '/' + appRouterMatch[1].replace(/\/$/, '')
                    : '/'
                  // Skip dynamic routes like [id] for auto-testing (they need params)
                  if (!routePath.includes('[')) {
                    if (!pageRoutes.includes(routePath)) {
                      pageRoutes.push(routePath)
                    }
                  }
                }

                // Match app/api/*/route.ts files
                const apiRouteMatch = path.match(/^app\/api\/(.+)\/route\.(tsx?|jsx?)$/)
                if (apiRouteMatch) {
                  const apiPath = '/api/' + apiRouteMatch[1]
                  if (!apiPath.includes('[')) {
                    apiRoutes.push(apiPath)
                  }
                }
              }
              ...
              for (const file of allFiles) {
                ...
                if (pagesMatch) {
                  const routeName = pagesMatch[1]
                  if (routeName === 'index') continue // Already have /
                  if (routeName === '_app' || routeName === '_document' || routeName === '_error') continue
                  if (routeName.startsWith('api/')) {
                    const apiPath = '/' + routeName
                    if (!apiPath.includes('[')) {
                      apiRoutes.push(apiPath)
                    }
                  } else {
                    const routePath = '/' + routeName.replace(/\/index$/, '')
                    if (!routePath.includes('[') && !pageRoutes.includes(routePath)) {
                      pageRoutes.push(routePath)
                    }
                  }
                }
              ...
              // Vite: try to detect routes from router configuration
              for (const file of allFiles) {
                const content = file.content || ''
                const path = file.path || ''
                ...
                if (content.includes('Route') || content.includes('createBrowserRouter') || content.includes('createRoutesFromElements')) {
                  // Extract path="..." from route definitions
                  const routeMatches = content.matchAll(/path\s*[:=]\s*["']([^"']+)["']/g)
                  for (const match of routeMatches) {
                    const routePath = match[1]
                    if (routePath && !routePath.includes(':') && !pageRoutes.includes(routePath)) {
                      pageRoutes.push(routePath.startsWith('/') ? routePath : '/' + routePath)
                    }
                  }
                }
              }
            ...
            // Add additional user-specified routes
            for (const route of additionalRoutes) {
              const normalizedRoute = route.startsWith('/') ? route : '/' + route
              if (normalizedRoute.startsWith('/api/') || normalizedRoute.startsWith('/api.')) {
                if (!apiRoutes.includes(normalizedRoute)) apiRoutes.push(normalizedRoute)
              } else {
                if (!pageRoutes.includes(normalizedRoute)) pageRoutes.push(normalizedRoute)
              }
            }

            // Deduplicate
            const uniquePageRoutes = [...new Set(pageRoutes)]
            const uniqueApiRoutes = [...new Set(apiRoutes)]

            logs.push(`[TEST] Discovered ${uniquePageRoutes.length} page routes: ${uniquePageRoutes.join(', ')}`)
            if (uniqueApiRoutes.length > 0) {
              logs.push(`[TEST] Discovered ${uniqueApiRoutes.length} API routes: ${uniqueApiRoutes.join(', ')}`)
            }

            // ──────────────────────────────────────────────────────────
            // STEP 2: Create sandbox and set up project
            // ──────────────────────────────────────────────────────────
            const sandbox = await createEnhancedSandbox({
              template,
              timeoutMs: 180000, // 3 minutes for full test suite
              env: {}
            })

            logs.push('[TEST] Sandbox created successfully')
            ...
            await sandbox.writeFiles(sandboxFiles)
            logs.push(`[TEST] Wrote ${sandboxFiles.length} files to sandbox`)
            ...
            logs.push(`[TEST] Installing dependencies with ${packageManager}...`)
            const installResult = await sandbox.installDependenciesRobust(workingDir, {
              timeoutMs: 90000,
              envVars: {},
              packageManager,
              ...
              onStderr: (data) => {
                const msg = data.trim()
                if (msg && !msg.includes('WARN') && !msg.includes('warning')) {
            ...
            logs.push('[TEST] Dependencies installed successfully')

            // For Expo projects, fix dependency versions
            if (isExpoProject) {
              try {
                await sandbox.executeCommand('npx expo install --fix', {
                  workingDirectory: workingDir,
                  timeoutMs: 120000,
                  envVars: {},
                  onStdout: (data) => logs.push(`[EXPO FIX] ${data.trim()}`),
                  onStderr: (data) => logs.push(`[EXPO FIX] ${data.trim()}`)
                })
              } catch (error) {
                logs.push('[TEST] Expo dependency fix skipped, continuing...')
              }
            }
            ...
            // ──────────────────────────────────────────────────────────
            logs.push('[TEST] Starting development server...')
            ...
            const devServerErrors: string[] = []

            const devCommand = isExpoProject ? "npx expo start --web" : "npm run dev"
            const devPort = isExpoProject ? 8081 : 3000
            const devTimeout = isExpoProject ? 60000 : 30000

            const devServer = await sandbox.startDevServer({
              command: devCommand,
              workingDirectory: workingDir,
              port: devPort,
              timeoutMs: devTimeout,
              envVars: isExpoProject ? { EXPO_NO_TELEMETRY: '1', EXPO_NO_REDIRECT: '1' } : {},
              onStdout: (data) => {
                const message = data.trim()
                logs.push(`[DEV] ${message}`)
                if (!serverStarted) {
                  // Detect server readiness for different frameworks
                  if (
                    message.includes('ready') ||
                    message.includes('listening') ||
                    message.includes('Local:') ||
                    message.includes('Started on') ||
                    (isExpoProject && message.includes('Waiting on http://localhost'))
                  ) {
                    serverStarted = true
                    serverReadyResolve()
                  }
              ...
              onStderr: (data) => {
                ...
                if (message &&
                  ...
                  !message.includes('should be updated for best compatibility')
                ) {
                  devServerErrors.push(message)
                  errors.push(`[DEV ERROR] ${message}`)
                }
                logs.push(`[DEV] ${message}`)
              },
            ...
            const serverTimeoutPromise = new Promise<void>((resolve) => {
              setTimeout(() => resolve(), isExpoProject ? 45000 : 20000)
            })
            await Promise.race([serverReadyPromise, serverTimeoutPromise])
            ...
            if (!serverStarted) {
              ...
              return {
                success: false,
                error: `Dev server failed to start within timeout (${projectType})`,
                logs,
                errors,
                devServerErrors,
                testResults: [],
            ...
            logs.push(`[TEST] Dev server started at ${baseUrl}`)

            // Give server a moment to fully initialize
            await new Promise(resolve => setTimeout(resolve, 3000))

            // ──────────────────────────────────────────────────────────
            // STEP 4: Test page routes
            // ──────────────────────────────────────────────────────────
            logs.push('[TEST] Testing page routes...')

            for (const route of uniquePageRoutes) {
              // Check if we're running low on time
              const currentTimeStatus = getTimeStatus()
              if (currentTimeStatus.isApproachingTimeout) {
                logs.push(`[TEST] Skipping remaining route tests due to time constraints`)
                break
              }
              ...
              try {
                const curlResult = await sandbox.executeCommand(
                  `curl -s -o /tmp/page_response.html -w "%{http_code}|%{time_total}" --max-time 15 "${url}"`,
                  {
                    workingDirectory: workingDir,
                    timeoutMs: 20000,
                    envVars: {}
                ...
                const responseTime = parseFloat(parts[1] || '0') * 1000 // Convert to ms

                if (statusCode >= 200 && statusCode < 400) {
                  // Check if response contains error indicators
                  const bodyCheck = await sandbox.executeCommand(
                    `cat /tmp/page_response.html | head -c 5000`,
                    { workingDirectory: workingDir, timeoutMs: 5000, envVars: {} }
                  )
                  const body = bodyCheck.stdout || ''
                  const hasErrorIndicators = body.includes('Internal Server Error') ||
                    body.includes('500') && body.includes('Error') ||
                  ...
                  if (hasErrorIndicators) {
                    testResults.push({
                      route, type: 'page', status: 'warning', statusCode,
                      message: `Page returned ${statusCode} but contains error indicators in HTML`,
                      responseTime: Math.round(responseTime)
                    })
                  } else {
                    testResults.push({
                      route, type: 'page', status: 'pass', statusCode,
                      message: `Page loaded successfully (${statusCode})`,
                      responseTime: Math.round(responseTime)
                    })
                  }
                } else {
                  testResults.push({
                    route, type: 'page', status: 'fail', statusCode,
            ...
            // ──────────────────────────────────────────────────────────
            // STEP 5: Test API routes (Next.js)
            // ──────────────────────────────────────────────────────────
            if (uniqueApiRoutes.length > 0) {
              logs.push('[TEST] Testing API routes...')

              for (const route of uniqueApiRoutes) {
                const currentTimeStatus = getTimeStatus()
                if (currentTimeStatus.isApproachingTimeout) {
                  logs.push(`[TEST] Skipping remaining API tests due to time constraints`)
                  break
                }
                ...
                try {
                  // Test GET request
                  const curlResult = await sandbox.executeCommand(
                    `curl -s -o /tmp/api_response.json -w "%{http_code}|%{time_total}" --max-time 10 "${url}"`,
                    { workingDirectory: workingDir, timeoutMs: 15000, envVars: {} }
                  )

                  const output = (curlResult.stdout || '').trim()
                  const parts = output.split('|')
                  const statusCode = parseInt(parts[0] || '0', 10)
                  const responseTime = parseFloat(parts[1] || '0') * 1000

                  // API routes: 200-499 are "expected" (even 404/405 means the route exists)
                  if (statusCode >= 200 && statusCode < 500) {
                    const bodyCheck = await sandbox.executeCommand(
                      `cat /tmp/api_response.json | head -c 2000`,
                      { workingDirectory: workingDir, timeoutMs: 5000, envVars: {} }
                    )

                    if (statusCode >= 200 && statusCode < 300) {
                      testResults.push({
                        route, type: 'api', status: 'pass', statusCode,
                        message: `API route responded successfully (${statusCode})`,
                        responseTime: Math.round(responseTime)
                      })
                    } else if (statusCode === 405) {
                      testResults.push({
                        route, type: 'api', status: 'pass', statusCode,
                        message: `API route exists but GET method not allowed (${statusCode}) - this is expected if route only handles POST/PUT/DELETE`,
                        responseTime: Math.round(responseTime)
                      })
                    } else {
                      testResults.push({
                        route, type: 'api', status: 'warning', statusCode,
                        message: `API route returned ${statusCode} - may need authentication or specific parameters`,
                        responseTime: Math.round(responseTime)
                      })
                    }
                  } else {
                    testResults.push({
                      route, type: 'api', status: 'fail', statusCode,
                      message: `API route returned server error (${statusCode})`,
                      responseTime: Math.round(responseTime)
                    })
                  }
                } catch (error) {
                  testResults.push({
                    route, type: 'api', status: 'fail',
            ...
            // ──────────────────────────────────────────────────────────
            // STEP 6: Check dev server errors accumulated during testing
            // ──────────────────────────────────────────────────────────
            const runtimeErrors = devServerErrors.filter(err =>
              !err.includes('ExperimentalWarning') &&
              !err.includes('DeprecationWarning') &&
              !err.includes('⚠') &&
              !err.includes('detected:') &&
              !err.includes('Warning')
            )

            if (runtimeErrors.length > 0) {
              logs.push(`[TEST] ${runtimeErrors.length} runtime errors detected during testing`)
              testResults.push({
                route: 'dev-server', type: 'runtime', status: 'fail',
                message: `${runtimeErrors.length} runtime errors detected: ${runtimeErrors.slice(0, 3).join('; ')}${runtimeErrors.length > 3 ? ` (and ${runtimeErrors.length - 3} more)` : ''}`
              })
            } else {
              testResults.push({
                route: 'dev-server', type: 'runtime', status: 'pass',
                message: 'No runtime errors detected during testing'
              })
            }
            ...
            const currentTimeStatus = getTimeStatus()
            if (!skipBuild && !currentTimeStatus.isApproachingTimeout) {
              logs.push('[TEST] Running production build...')

              if (isExpoProject) {
                // Expo: run TypeScript check
                const hasTypeScriptFiles = allFiles.some((f: any) =>
                  f.path.endsWith('.ts') || f.path.endsWith('.tsx')
                )
                ...
                if (hasTypeScriptFiles && allFiles.some((f: any) => f.path === 'tsconfig.json')) {
                  const typeCheckResult = await sandbox.executeCommand("npx tsc --noEmit", {
                    ...
                    onStderr: (data: string) => {
                      const msg = data.trim()
                      if (msg && !msg.includes('warning') && !msg.includes('Warning')) {
                  ...
                  buildSuccess = typeCheckResult.exitCode === 0
                } else {
                  logs.push('[BUILD] No TypeScript files found, skipping type check')
                }
              } else {
                // Vite/Next.js: run npm run build
                const buildResult = await sandbox.executeCommand("npm run build", {
                  workingDirectory: workingDir,
                  timeoutMs: 90000,
                  envVars: {},
              ...
              if (buildSuccess) {
                testResults.push({
                  route: 'build', type: 'build', status: 'pass',
                  message: 'Production build completed successfully'
                })
                logs.push('[TEST] Build completed successfully')
              } else {
                testResults.push({
                  route: 'build', type: 'build', status: 'fail',
                  message: `Build failed: ${buildErrors.slice(0, 3).join('; ')}${buildErrors.length > 3 ? ` (and ${buildErrors.length - 3} more errors)` : ''}`
                })
                logs.push('[TEST] Build FAILED')
              }
            } else if (skipBuild) {
              logs.push('[TEST] Build check skipped (skipBuild=true)')
            } else {
              logs.push('[TEST] Build check skipped due to time constraints')
              testResults.push({
                route: 'build', type: 'build', status: 'warning',
                message: 'Build check skipped due to time constraints'
              })
            }
            ...
            return {
              success: allPassed,
              summary: allPassed
                ? `All ${totalTests} tests passed! App is fully functional.`
                : `${failCount} of ${totalTests} tests failed. ${warningCount} warnings.`,
              projectType,
              testResults,
              stats: {
                total: totalTests,
              ...
              pageRoutesTested: uniquePageRoutes,
              apiRoutesTested: uniqueApiRoutes,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error'
            console.error(`[ERROR] test_app failed:`, error)
            ...
            if (error && typeof error === 'object') {
              ...
              if (sandboxError.result) {
                ...
                stdout = result.stdout || ''
                stderr = result.stderr || ''
                detailedError = `Test execution failed: exit status ${exitCode || 'unknown'}\n\nSTDOUT:\n${stdout}\n\nSTDERR:\n${stderr}`
              } else if (sandboxError.originalError && sandboxError.originalError.result) {
                const result = sandboxError.originalError.result
                exitCode = result.exitCode || null
                stdout = result.stdout || ''
            ...
            return {
              success: false,
              error: `App testing failed: ${detailedError}`,
              exitCode,
              stdout,
              stderr,
              fullErrorDetails: {
