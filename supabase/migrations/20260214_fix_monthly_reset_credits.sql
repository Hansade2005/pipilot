...
-- New values: Free=150, Creator=1000, Collaborate=2500, Scale=5000
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN w.current_plan = 'scale' THEN 15000
      ELSE 0
    END,
    'monthly_reset',
...
BEGIN
  ...
  VALUES (NEW.id, 150, 'free')
  ON CONFLICT (user_id) DO NOTHING;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION public.reset_monthly_credits() IS 'Reset monthly credit usage and grant new credits based on plan. Values: Free=150, Creator=1000, Collaborate=2500, Scale=5000';
